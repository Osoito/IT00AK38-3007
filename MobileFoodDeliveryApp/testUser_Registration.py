class UserRegistration:
    ...
    def is_valid_email(self, email):
        ...
        This rule only checks that the email contains exactly one '@' symbol and has a '.' in the domain part.
        ...
        # Ensure domain does not contain '@' and both local and domain are non-empty
        return "." in domain and local != "" and domain != "" and "@" not in domain
...
class TestUserRegistration(unittest.TestCase):
    ...
    def test_initialization(self):
        """
        Test that UserRegistration initializes with an empty users dictionary.
        """
        reg = UserRegistration()
        self.assertIsInstance(reg.users, dict)
        self.assertEqual(len(reg.users), 0)

    def test_is_valid_email_edge_cases(self):
        """
        Test edge cases for is_valid_email method.
        """
        # Multiple '@' symbols
        self.assertFalse(self.registration.is_valid_email("user@@example.com"))
        # No '.' in domain part
        self.assertFalse(self.registration.is_valid_email("user@examplecom"))
        # Valid email with subdomain
        self.assertTrue(self.registration.is_valid_email("user@mail.example.com"))
        # Empty string
        self.assertFalse(self.registration.is_valid_email(""))

    def test_is_strong_password_edge_cases(self):
        """
        Test edge cases for is_strong_password method.
        """
        # Only numbers
        self.assertFalse(self.registration.is_strong_password("12345678"))
        # Only letters
        self.assertFalse(self.registration.is_strong_password("abcdefgh"))
        # Less than 8 characters
        self.assertFalse(self.registration.is_strong_password("a1b2c3"))
        # Valid strong password
        self.assertTrue(self.registration.is_strong_password("abc12345"))
        # Empty string
        self.assertFalse(self.registration.is_strong_password(""))

    def test_users_dict_mutation(self):
        """
        Test direct mutation of users dictionary.
        """
        self.registration.users["test@example.com"] = {"password": "Password123", "confirmed": False}
        self.assertIn("test@example.com", self.registration.users)
        self.assertFalse(self.registration.users["test@example.com"]["confirmed"])
