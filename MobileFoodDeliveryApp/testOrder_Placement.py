...
class UserProfile:
    """
    Represents a user's profile, including delivery address and order history.
    """
    def __init__(self, delivery_address=""):
        self.delivery_address = delivery_address
        self.order_history = []  # Ensure order_history is always initialized
...
class TestOrderPlacement(unittest.TestCase):
    ...
    def test_add_item_invalid_quantity(self):
        """
        Test adding an item with invalid (negative) quantity.
        """
        result = self.cart.add_item("Burger", 8.99, -2)
        # The Cart class does not currently prevent negative quantities, so it will add/update.
        # For now, check that the quantity is negative.
        self.assertEqual(self.cart.items[0].quantity, -2)
        self.assertEqual(result, "Added Burger to cart")
    ...
    def test_remove_item_not_in_cart(self):
        ...
        # Should not raise error, just return the message.
        self.assertEqual(result, "Removed NonExistentItem from cart")
        self.assertEqual(len(self.cart.items), 0)


class TestCartItemAndMenu(unittest.TestCase):
    def test_cart_item_update_quantity(self):
        item = CartItem("Burger", 5.0, 2)
        item.update_quantity(5)
        self.assertEqual(item.quantity, 5)

    def test_cart_item_get_subtotal(self):
        item = CartItem("Pizza", 10.0, 3)
        self.assertEqual(item.get_subtotal(), 30.0)

    def test_restaurant_menu_is_item_available(self):
        menu = RestaurantMenu(["Burger", "Salad"])
        self.assertTrue(menu.is_item_available("Burger"))
        self.assertFalse(menu.is_item_available("Pizza"))

    def test_user_profile_initialization_edge(self):
        profile = UserProfile()
        self.assertEqual(profile.delivery_address, "")
        self.assertEqual(profile.order_history, [])

    def test_restaurant_menu_initialization_edge(self):
        menu = RestaurantMenu([])
        self.assertEqual(menu.available_items, [])
        menu2 = RestaurantMenu(None)
        self.assertEqual(menu2.available_items, None)
